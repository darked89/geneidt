...
void readargv(int argc, char *argv[],
    ...
    if (optind < argc) {
        ...
        sprintf(mess, "Two filenames are needed: DNA sequence(fasta format) and features (GFF format)\n%s", USAGE);
        printError(mess);
    }
    ...
    if (optind < argc) {
        ...
        if (optind < argc) {
            sprintf(mess, "Two  filenames required but more than two arguments provided\n%s", USAGE);
            printError(mess);
        }
    }
    else {
