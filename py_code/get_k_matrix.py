...
observed_kmers_count_dict = {}
for position in range(1,seq_len_in + 1):
    observed_kmers_count_dict[position] = 0
...
for line in open(in_fn).readlines():
    ...
    if discard != 1:
        ...
        for i in range(0, seq_len_real - kmer_plus_size + 1):
            ...
            position = i+1
            observed_kmers_count_dict[position] += 1 
...
##observed_kmers_num = 3465
for position in ktuple_plus_dict.keys():
    observed_kmers_num = observed_kmers_count_dict[position]
    for list_kmer in kmer_plus_list2:
        first_nucl = list_kmer[0]
        ##observed_kmers_num += ktuple_plus_dict[position][list_kmer]
        kmer_plus_count  = ktuple_plus_dict[position][list_kmer]
        first_nucl_count = ktuple_dict[position][first_nucl]
        pseudo_count = (kmer_plus_count +1)/(first_nucl_count + 4)
        print position, list_kmer, pseudo_count #, kmer_count
