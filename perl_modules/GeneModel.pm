...
sub eraseModel{
    ...
    $self->{_model}->{IntragenicRules2} = undef;
...
sub addIntragenicRule2{
    my ($self, $uc, $de, $minI, $maxI, $block) = @_;
    push @{$self->{_model}->{IntragenicRules2}},{uc=>$uc, de=>$de, mi=>$minI, Mi=>$maxI, block=>$block};
}
sub intronRange{
    my ($self, $min, $max) = @_;
...
sub useDefault{
    ...
    @uc = (qw(First+));
    @de = (qw(Intron+));
    $self->addIntragenicRule2([@uc],[@de],$minI,$maxI,$block);
    @uc = (qw(Internal+));
    @de = (qw(Intron+));
    $self->addIntragenicRule2([@uc],[@de],$minI,$maxI,$block);
    @uc = (qw(Intron+));
