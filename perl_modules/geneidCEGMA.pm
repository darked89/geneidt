...
# Based on same tool from HMMstar.pm (Ian Korf).
# Copied from CEGMA (Ian Korf/Genis Parra)
...
package geneidCEGMA::SequenceModel;

sub new {
    my ($class, $name, $type, $order, $seqs, $pseudo, $frame) = @_;
    print STDERR " name: $name; type:$type; order:$order; pseudo:$pseudo; seqs: $seqs; frame=$frame";
    my $self = bless {
    name  => $name,
    type  => $type,
    order => $order,
    ...
    #from pseudo and below DK 20141202
    if ($type eq 'MM') {
...
sub frequencies_model{
   ...
   # print STDERR "\n(IN FUNCTION: $order, $seqs, $pseudo, $frame) \n";
    $pseudo = 0 unless defined $pseudo;
    my @alph = (qw(A C G T));
    my $table = geneidCEGMA::dna_word_table($order, "{}");
    my @total = (0, 0, 0);
    ...
    foreach my $seq (@$seqs) {
     #print STDERR "\n(IN FUNCTION: order: $order -1; \n";
    my $fr=0;
     my $len=length($seq);
     #print STDERR "\n(IN FUNCTION: length: $len order: $order -1; \n";
...
sub pwm_model{
    ...
    $pseudo = 0 unless defined $pseudo;
    my @alph = (qw(A C G T));
    my $table = geneidCEGMA::dna_word_table($order+1, "{}");
